...
def check_if_already_scraped(alphabet, csv_file):
    try:
        ...
        with open(csv_file, 'r', newline='', encoding='utf-8') as file:
            ...
            for row in reader:
                if 'name' in row and row['name'].lower().startswith(alphabet.lower()):
                    return True
...
def filter_and_save_drugs(drugs, alphabet, csv_file):
    ...
    already_scraped = check_if_already_scraped(alphabet, csv_file)
    if already_scraped:
        print(f"Drugs starting with '{alphabet}' have already been scraped.")
        return

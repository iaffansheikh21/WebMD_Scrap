...
async function scrapeVitaminsByAlphabet(alphabet) {
    ...
    try {
        ...
        const csvFilename = 'vitamins_details.csv';
        const stream = fs.createWriteStream(csvFilename, { flags: 'a' });
        stream.write('Vitamin,Overview,Uses & Effectiveness,Side Effects,Precautions & Warnings,Interactions,Dosing\n');
        ...
        for (const linkObj of vitaminLinks) {
            try {
                ...
                console.log(`Vitamin Name: ${vitaminName}`);
                console.log(`Overview: ${overview}`);
                console.log(`Uses & Effectiveness: ${usesEffectiveness}`);
                console.log(`Side Effects: ${sideEffects}`);
                console.log(`Precautions & Warnings: ${precautionsWarnings}`);
                console.log(`Interactions: ${interactions}`);
                console.log(`Dosing: ${dosing}`);
                console.log('-------------------------');

                stream.write(`${vitaminName},${overview},${usesEffectiveness},${sideEffects},${precautionsWarnings},${interactions},${dosing}\n`);
            } catch (error) {
